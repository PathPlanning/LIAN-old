...
bool Config::getConfig(const char *FileName) {
    ...
    if (angle == 0) {
        std::cout << "Warning! '" << CNS_TAG_ANGLELIMIT << "' is zero. It's set to " << CN_PTD_AL << "." << std::endl;
        angle = CN_PTD_AL;
    }
    ...
    if (distance == 0) {
        std::cout << "Warning! Wrong '" << CNS_TAG_DISTANCE << "' section. It's set to " << CN_PTD_D << "."
                  << std::endl;
        distance = CN_PTD_D;
    }
    ...
    if (!element) {
        std::cout << "Error! No '" << CNS_TAG_WEIGHT << "' element found inside '" << CNS_TAG_ALGORITHM
                  << "' section. It's set to " << CN_PTD_W << "." << std::endl;
        weight = CN_PTD_W;
    } else {
        value = element->GetText();
        stream << value;
    ...
    if (!element) {
        std::cout << "No '" << CNS_TAG_STEPLIMIT << "' element found inside '" << CNS_TAG_ALGORITHM
                  << "' section. It's set to 0. which means unlimited number of steps" << std::endl;
        steplimit = 0;
    } else {
        value = element->GetText();
        if (value[0] == '-') {
    ...
    if (!element) {
        std::cout << "No '" << CNS_TAG_CRADIUSFACTOR << "' element found inside '" << CNS_TAG_ALGORITHM
    ...
    if (!element) {
        std::cout << "No '" << CNS_TAG_DECRDISTFACTOR << "' element found inside '" << CNS_TAG_ALGORITHM
                  << "' section. It's set to " << CN_PTD_DDF << "." << std::endl;
    ...
    if (!element) {
        std::cout << "No '" << CNS_TAG_DISTANCEMIN << "' element found inside '" << CNS_TAG_ALGORITHM
                  << "' section. It's set to " << std::min(CN_PTD_DMIN, distance) << "." << std::endl;
        distanceMin = std::min(CN_PTD_DMIN, distance);
    } else {
        value = element->GetText();
        stream << value;
        stream >> distanceMin;
    ...
    if (!element) {
        std::cout << "No '" << CNS_TAG_LINECOST << "' element found inside '" << CNS_TAG_ALGORITHM
                  << "' section. It's set to " << CN_PTD_LC << "." << std::endl;
        linecost = CN_PTD_LC;
    } else {
        value = element->GetText();
        stream << value;
    ...
    if (!element) {
        std::cout << "No '" << CNS_TAG_NOFPTOINCRAD << "' element found inside '" << CNS_TAG_ALGORITHM
                  << "' section. It's set to " << CN_PTD_NOFPTOINCRAD << "." << std::endl;
